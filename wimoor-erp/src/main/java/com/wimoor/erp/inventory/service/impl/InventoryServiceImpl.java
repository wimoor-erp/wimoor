...
public class InventoryServiceImpl  extends ServiceImpl<InventoryMapper,Inventory> implements IInventoryService {
	...
	public void stockTakingInvOperate(StockTaking stocktaking, String warehouseid, UserInfo user) throws BizException {
		...
		Warehouse warehouse = warehouseService.getById(warehouseid);// 获取仓库下面的仓位
			List<Map<String, Object>> list = stockTakingService.getItemList(stocktaking.getId(),warehouse.getId(),null);
			if (list != null && list.size() > 0) {
				...
				if (skulist.size() > 0) {
					...
					for (int j = 0; j < list.size(); j++) {
						...
						invpara.setWarehouse(warehouse.getId());
