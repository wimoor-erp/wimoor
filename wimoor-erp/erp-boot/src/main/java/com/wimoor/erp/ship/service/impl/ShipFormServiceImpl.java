...
public class ShipFormServiceImpl implements IShipFormService{
		...
		for (int i = 0; i < dto.getList().size(); i++) {
			...
			if(materialid==null) {
			 ...
			 if(fulfillable<shipqty) {
				 ...
				 assemblyFormService.createAssemblyFormByShipment(user,warehouseid,material,dto.getFormid(),needassembly); 
			 }
	...
	public void subFulfillableToOutboundChange(String userid, String shopid,String number,String formid,String warehouseid, String material, Integer amount) {
		InventoryParameter para = new InventoryParameter();
		para.setAmount(amount);
		para.setMaterial(material);
		EnumByInventory statusinv = EnumByInventory.Ready;
	...
	public String updateDisable(UserInfo user, ShipFormDTO dto) {
		// TODO Auto-generated method stub
				...
				if(dto.getOpttype()!=null&&dto.getOpttype().contains("cancelassembly")) {
					assemblyFormService.cancelByForm(user,dto.getFormid());
				}
			return "success";
	}
		...
		for (int i = 0; i <itemlist.size(); i++) {
			...
			Integer qty = item.getQuantity();
				if (qty > 0) {// 修改数量大于计划数量,changeadd
					addFullfillableToOutboundChange(user.getId(),user.getCompanyid(),
							dto.getNumber(),dto.getFormid(),dto.getWarehouseid(), item.getMaterialid(),  qty);
				} else if (qty < 0){// 修改数量小于计划数量,changesub
					subFulfillableToOutboundChange(user.getId(),user.getCompanyid(),  
							dto.getNumber(),dto.getFormid(),dto.getWarehouseid(),item.getMaterialid(), qty*-1);
				}
		}
	}
