...
public class WarehouseServiceImpl extends  ServiceImpl<WarehouseMapper,Warehouse> implements IWarehouseService {
	...
	final ShipPlanMapper shipPlanMapper;
	...
	//获取仓库下面的子仓位函数 递归生成
	public List<Warehouse> recursionWarehouseSelectList(UserInfo user,String parentId, List<Warehouse> wareList) {
        ...
        Optional.ofNullable(wareList).orElse(new ArrayList<>())
            ...
            .forEach(warehouse -> {
            	...
            	try {
            		...
            		if(Result.isSuccess(result)&&result.getData()!=null&&result.getData().get("name")!=null) {
	...
	public Integer deleteInfoById(String id) throws ERPBizException {
		...
		if (warehouse != null) {
			...
			QueryWrapper<ShipPlan> queryWrapperShipPlan = new QueryWrapper<ShipPlan>();
			queryWrapperShipPlan.eq("warehouseid", warehouse.getId());
			List<ShipPlan> shiplist = shipPlanMapper.selectList(queryWrapperShipPlan);
			if (shiplist != null && shiplist.size() > 0) {
				throw new ERPBizException("该仓库还存在着补货计划！");
			}
