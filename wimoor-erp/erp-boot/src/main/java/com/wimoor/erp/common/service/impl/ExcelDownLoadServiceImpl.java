...
public class ExcelDownLoadServiceImpl implements IExcelDownLoadService{
	...
	@Override
	public void uploadMaterialBaseInfoFile(UserInfo user, Row info,String mtype) {
		String sku = null;
		if (info.getCell(0) != null) {
	...
	public void uploadMaterialSupplierFile(UserInfo user, Row info) {
		...
		if(supplierlist!=null && supplierlist.size()>0) {
			...
			if(material!=null) {
				//先删除关系 再添加
				QueryWrapper<MaterialSupplier> supqueryWrapper=new QueryWrapper<MaterialSupplier>();
				supqueryWrapper.eq("materialid", material.getId());
				supqueryWrapper.eq("isdefault", 1);
				materialSupplierMapper.delete(supqueryWrapper);
				String procode=null;
				if (info.getCell(2) != null) {
					info.getCell(2).setCellType(CellType.STRING);
