							  ...
							  <div class="s-g-footer">
								  <el-row>
									  <el-col :span="24">
										 ...
										 <el-select v-loading="timeLoading" v-model="item.shipment.deliveryWindowOptionId" @change="saveTime(item.shipment)">
										 												 <el-option 
									  ...
									  <div class="flex-center-between font-base" >
										  ...
										  <span>
											  货件类型
											<el-select :disabled="item.plan.auditstatus!=6"  @change="updateShipmentTranStyle(item.shipment.shipmentid,item.shipment.transtyle)" v-model="item.shipment.transtyle" placeholder="货件类型" style="width: 240px">
...
<script setup>
  ...
  function updateShipmentTranStyle(id,transtyle){
    shipmentPlacementApi.updateShipmentTranStyle({"shipmentid":id,"transtyle":transtyle}).then((res)=>{
      if(res.data){
	...
	function handleOperationChange(data){
       ...
       loadData();
			...
			boxShipment()
		}else if(data && data.operation=="confirmDeliveryWindowOptions"){
