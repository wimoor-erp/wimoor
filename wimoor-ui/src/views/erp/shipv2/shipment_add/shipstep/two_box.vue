<template>
	<div class="box-margin">
		<div class="pag-radius-bor mar-bot" >
          ...
          <el-tabs v-else  v-model="selectPackingOptionId"   type="border-card" class="demo-tabs" @tab-click="handleClick">
            <el-tab-pane v-for="(optitem,index) in options"   :name="optitem.packingOptionId">
              <template #label>
                <span class="custom-tabs-label">
                  装箱方案 {{index+1}} ({{optitem.packingGroups.length}}个装箱组)
                  <span v-if="optitem.status=='ACCEPTED'">[已选择]</span>
                  <div  v-if="optitem.discounts!=null&&optitem.discounts.length>0">
                       <div class="text-green" v-for="(item,index) in optitem.discounts">{{getCurrencyMark(item.value.code)}}{{item.value.amount}}预计包装折扣</div>
                  </div>
                  <div v-else>
                       <span class="font-extraSmall">无配送折扣
                        <el-tooltip content="这种标准包装方式不会优化库存分布,也不能让您享受到配送送折扣" placement="top">
                         <el-icon class="pointer"><InfoFilled /></el-icon>
                        </el-tooltip>
                       </span>
				  ...
				  <div class="flex-between boxgroup-wrap"   >
                         ...
                         <el-space :size="40">
                           <div>{{groupInfo.skunum}}个SKU (共{{groupInfo.skuqty}}件商品)</div>
                          <div> 体积:<span class="font-black">{{groupInfo.volume}}m³</span></div>
                           <div>重量:<span class="font-black">{{groupInfo.weight}}KG</span></div>
                           <div>材积:<span class="font-black">{{groupInfo.cbm}}KG</span></div>
                         </el-space>
                      </div>
										  </el-col>
										  <el-col :span="12">
                          ...
                          <el-space>
                            <el-link type="success" @click="downloadGroupInfo(groupInfo,index)">下载装箱模版</el-link>
                            <el-link type="primary" @click="showGroupInfo(groupInfo)">查看商品详情</el-link>
                          </el-space>
                         </div>
										  </el-col>
				 						</el-row>
...
<script setup>
	...
	import {dateFormat,formatFloat,getValue,formatPercent,CheckInputIntLGZero,CheckInputFloat,getCurrencyMark} from '@/utils/index.js';
    ...
    selectPackingOptionId,packagetitle}=toRefs(state);
	...
	function listPackingOptions(nextToken){
        ...
        state.options.forEach(item=>{
          if(item.status==="OFFERED"){
            state.selectPackingOptionId=item.packingOptionId;
          }
        })
        if(!nextToken&&!state.selectPackingOptionId){
          state.selectPackingOptionId=state.options[0].packingOptionId;
        }
	...
	async function listPackingGroupItems(packingOptionId,packingGroupId,nextToken,optionitem){
      ...
      var cbm=0;
          ...
          console.log(item.skuinfo);
          if(item.skuinfo.weight){
            weight+=item.skuinfo.weight*item.quantity;
          }
          if(item.skuinfo.skuvolume){
            volume+=item.skuinfo.skuvolume*item.quantity;
          }
          if(item.skuinfo.pkglength&&item.skuinfo.pkgwidth&&item.skuinfo.pkgheight){
            cbm+=(item.skuinfo.pkglength*item.skuinfo.pkgwidth*item.skuinfo.pkgheight)/5000*item.quantity;
          }
       ...
       if(volume){
         groupInfo.volume=volume.toFixed(3);
       }
       if(weight){
         groupInfo.weight=weight.toFixed(3);
       }
       if(cbm){
         groupInfo.cbm=cbm.toFixed(3);
       }
