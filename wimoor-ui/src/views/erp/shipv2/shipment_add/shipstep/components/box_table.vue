<template>
	...
	<template #footer>
      ...
      <el-button  style="margin-left: 10px;"   @click="downloadBoxInfo()">下载箱子详情</el-button>
		</div>
...
<!--			 <el-button type="success" @click.stop="downloadTemp" plain>下载模板</el-button>-->
			 ...
			 <el-button type="primary" :loading="btnloading" @click.stop="uploadExcel">
			   上传文件
			 </el-button></div></div>
		   </span>
    </template>
...
<script setup>
  ...
  import purchaseEditApi from "@/api/erp/purchase/form/editApi.js";
	...
	boxListData,boxVisiable,boxDetail,planData,notice,boxtitleVis,sliderMax,affixState,uploadVisible,myfile}=toRefs(state);
...
function loadBoxData(){
  ...
  function downloadBoxInfo(){
    shipmentplanApi.downExcelBoxDetail({
      "formid":formid
    }).then(res => {
  ...
  function uploadExcel() {
    ...
    FormDatas.append('formid', formid);
    FormDatas.append('packingGroupId', state.operatorInfo.packingGroupId);
    shipmentplanApi.uploadExcelBoxDetail(FormDatas).then((res) => {
      ElMessage.success("箱子信息保存成功！");
      emit('change', state.operatorInfo, state.planData);
      getBoxDetial();
      state.btnloading = false;
