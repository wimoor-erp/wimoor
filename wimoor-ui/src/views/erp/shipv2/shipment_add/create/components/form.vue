		...
		<el-row>
            ...
            <el-col :span="8">
              <el-form-item label="运输方式">
                <el-select v-model="formData.transtyle">
...
let state=reactive({
			...
			totalproducts:0 ,
			addrIndex:3,
			formData:{name:"",groupid:"",warehouseid:"",marketplaceid:"",transtyle:"SP",
...
function loadInplace(){
 ...
 watch(() =>router.currentRoute.value.query,(newValue,oldValue)=> {
         ...
         state.marketList.forEach(item=>{
        	if(item.marketplaceid===val){
        		state.formData.amazonauthid=item.amazonauthid;
        	}
        })
