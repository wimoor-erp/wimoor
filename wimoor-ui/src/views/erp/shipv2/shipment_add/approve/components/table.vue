<template>
	<div class="text-right" v-if="planData && (!planData.checkInv)&&parseInt(planData.invstatus)<2" style="margin-bottom:10px"> 
	...
	<el-card shadow="hover" style="margin-bottom: 10px;">
		...
		<el-table :data="itemlist" border style="width: 100%;margin-top:16px;" :height="planData && (!planData.checkInv)&&parseInt(planData.invstatus)<2?'calc(100vh - 330px)':'calc(100vh - 290px)'" :scrollbar-always-on="true">
		  ...
		  <el-table-column prop="quantity" label="发货数量"   width="150" >
			  <template #default="scope">
				  <el-input-number v-if="scope.row.editQuantity" v-model="scope.row.quantity" :min="0" size="small" style="width: 100px" />
				  <span v-else>{{scope.row.quantity}}</span>
			  </template>
			  </el-table-column>
		  <el-table-column prop="quantity"  v-if="planData&&(planData.auditstatus==10 || planData.auditstatus==1 )" label="操作"  width="110"   >
		      <template #default="scope">
		          <el-button v-if="!scope.row.editQuantity" class='el-button--blue' @click="handleEditQuantity(scope.row)">修改数量</el-button>
		          <el-button v-else class='el-button--blue' @click="handleSaveQuantity(scope.row)">保存</el-button>
		      </template>
		  </el-table-column>
		</el-table>
	</el-card>
...
<script setup>
	...
	import MateiralDialog from "@/views/erp/shipv2/shipment_add/create/components/mateiralDialog.vue";	
			...
			function handleEditQuantity(row){
				row.oldQuantity = row.quantity;
				row.editQuantity = true;
			}
			
			function handleSaveQuantity(row){
				if(row.quantity < 0){
					ElMessage.error('发货数量不能小于0');
					row.quantity = row.oldQuantity;
					return;
				}
				var param={
					"id":row.id,
				...
				shipmentplanApi.updatePlanItem(param).then(res=>{
						ElMessage.success('修改成功！');
