<template>
		...
		<el-col :span="24">
			<div class="gird-line-head el-white-bg">
				...
				<div class="rt-btn-group">
					...
					<el-button type="success" v-if="plandata.auditstatus==1" :loading="submitLoading"  @click="approveAndKeepPlan">
						<el-icon><Stamp /> </el-icon> 审核并保持</el-button>
					<el-button type="success"   disabled v-if="plandata.auditstatus!=7 && plandata.auditstatus!=11&& plandata.auditstatus!=1  &&plandata.auditstatus!=10 &&!plandata.iserror"  >已审核</el-button>
					<el-button type="info"   disabled v-if="plandata.auditstatus==11"  >已驳回</el-button>
					<el-button type="warning"   disabled v-if="plandata.auditstatus==10"  >已保持</el-button>
					<el-button type="danger"   disabled v-if="plandata.auditstatus!=7 && plandata.auditstatus!=11&& plandata.auditstatus!=1 &&plandata.auditstatus!=10 &&plandata.iserror"   >审核失败</el-button>
					<Helper name="货件处理" />
				</div>
	...
	<el-dialog v-model="approveVisible" title="发货货件审核"  :destroy-on-close='true' width="60%" @close='closeDialog'>
			 ...
			 <el-table :data="shipmentList" border style="width: 100%;margin-top:16px;" >
				 ...
				 <el-table-column prop="toquantity" label="发货数量"   >
					 <template #default="scope">
...
<script setup>
	...
	import calculateApi from "@/api/amazon/profit/calculateApi";
			...
			function approveAndKeepPlan(){
				 ElMessageBox.confirm('确认审核并保持该计划?', '警告', {
					    distinguishCancelAndClose: true,
						...
						center: true,
				 		...
				 		.then(() => {
							 submitLoading.value=true;
				 		  shipmentplanApi.approveAndKeepInboundPlan({"id":plandata.value.id}).then(res=>{
				 			ElMessage.success('审核并保持成功');
							 submitLoading.value=false;
				 			 loadData();
				 		  }).catch(error=>{
							   submitLoading.value=false;
