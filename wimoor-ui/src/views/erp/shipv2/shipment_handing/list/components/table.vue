	   ...
	   style="width: 100%;margin-bottom:16px;">
	    ...
	    <el-table-column prop="skuamount" label="SKU个数" width="100"    sortable="custom">
          ...
          <div>  <el-space>
					 ...
					 <div ><el-tag  v-if="scope.row.areCasesRequired" type="success"  size="small">原装</el-tag></div>
            </el-space>
          </div>
            <el-space class="font-extraSmall pointer guidspace">
              <el-icon  title="电子订单" :class="scope.row.ordguid?'text-green':'disabled'"   @click="handleCustomsXml(scope.row,'ordguid')"><Bicycle /></el-icon>
              <el-icon   title="跨境电商出口清单" :class="scope.row.invguid?'text-green':'disabled'" @click="handleCustomsXml(scope.row,'invguid')" ><Van /></el-icon>
              <el-icon  title="跨境报关单" :class="scope.row.decguid?'text-green':'disabled'" @click="handleCustomsXml(scope.row,'decguid')" ><Ship /></el-icon>
            </el-space>
...
<script setup>
	...
	const emit = defineEmits(["change","customsChange"]);
    ...
    params.shipmentid=parmashead.value.shipmentids;
  ...
  function handleCustomsXml(row,type){
		 emit("customsChange",{row:row,type:type});
  }
