<template>
	<div class="box-ship">
		...
		<div class="con-body"  >
          ...
          <el-button     @click="downExcelBoxTemp()">下载装箱模版</el-button>
				</div>
			</el-col>
				<el-col :span="4"  v-if="!isview">
					<div class="rt-btn-group">
            <el-button    type="primary"  @click="uploadVisible=true;">上传装箱信息</el-button>
...
<!--			 <el-button type="success" @click.stop="downloadTemp" plain>下载模板</el-button>-->
			 ...
			 <el-button type="primary" :loading="submitloading" @click.stop="uploadExcel">
			   上传文件
			 </el-button></div></div>
		   </span>
    </template>
...
<script setup>
  ...
  function downExcelBoxTemp(){
    let planData=JSON.parse(JSON.stringify(state.planData.shipmentAll));
    let itemList=[];
      state.tableData.forEach(item=>{
      let itemData={sku:item.sellersku,quantity:item.quantityshipped};
      itemList.push(itemData);
    })
    planData.countryname=planData.country;
    planData.itemlist=itemList;
    shipmentBoxApi.downExcelBoxCaseTemp(planData,state.planData.plan.number+'-'+state.planData.shipment.shipmentConfirmationId+".xlsx");
  }
  function uploadExcel() {
    state.submitloading = true;
    let FormDatas = new FormData();
    FormDatas.append('file', state.myfile);
    FormDatas.append('formid', state.planData.shipment.formid);
    FormDatas.append('shipmentid', state.planData.shipment.shipmentid);
    shipmentplanApi.uploadShipmentExcelBoxDetail(FormDatas).then((res) => {
      ElMessage.success("箱子信息保存成功！");
      emit('change');
      state.submitloading=false;
