<template>
  <div class="customs-wrap">
    <el-table :data="productData.list"  id="customs-declaration-table" border style="width: 100%;" >
      ...
      <el-table-column prop="quantityshipped" label="申报数量" width="120"   >
        <template #default="scope">
          <div>{{scope.row.quantityshipped}} <span  class="font-extraSmall">（{{formatFloat(scope.row.quantityshipped*scope.row.weight)}}kg）</span> </div>
    ...
    <el-row >
      ...
      <div class="rt-btn-group m-t-8">
        <el-space>
          ...
          <el-button :disabled="isEdit" type="primary" @click.stop="showCustomsXml">
            下载电商订单
          </el-button>
          <el-button :disabled="isEdit"  type="primary"  @click.stop="showCustomsInventoryXml" >
            下载电商出口清单
          </el-button>
          <el-button :disabled="isEdit"  type="primary"  @click.stop="showCustomsDeclarationXml" >
            下载出口报关单
          </el-button>
        </el-space>
      </div>
  ...
  <CustomsOrderXml ref="customsXmlRef" />
  <CustomsDeclarationXml ref="customsDeclarationXmlRef" />
  <CustomsInventoryXml ref="customsInventoryXmlRef" />
</template>
...
import CustomsOrderXml from "./customs_order_xml.vue";
import CustomsDeclarationXml from "./customs_declaration_xml.vue";
import CustomsInventoryXml from "./customs_inventory_xml.vue";
...
const customsDeclarationXmlRef=ref(null);
const customsInventoryXmlRef=ref(null);
...
let{ uploadVisible,uploadloading,uploadTitle,myfile,tableData,search,templatetitle,isEdit,
  rateVisible,rateTableData,countryList,wrapTypeValue,unitOptions,
}=toRefs(state);
function changeWrapType(){
  console.log(wrapTypeValue.value);
}

function downloadLabel(ftype){
  downloadShipmentLabel(shipInfo.value.shipmentid,ftype,null);
}
function showCustomsXml(){
  customsXmlRef.value.show(shipInfo.value.plan.groupid,shipInfo.value.shipment.shipmentConfirmationId,shipInfo.value.shipment.ordguid);
}
function showCustomsDeclarationXml(){
  customsDeclarationXmlRef.value.show(shipInfo.value.plan.groupid,shipInfo.value.shipment.shipmentConfirmationId,shipInfo.value.shipment.decguid);
}
function showCustomsInventoryXml(){
  customsInventoryXmlRef.value.show(shipInfo.value.plan.groupid,shipInfo.value.shipment.shipmentConfirmationId,shipInfo.value.shipment.invguid);
}
...
function saveCustoms(){
  ...
  shipmentTransportationApi.saveCustomList(productData.list).then(res=>{
...
async function loadCustoms(shipmentid,isreload){
  await shipmentTransportationApi.getCustomList({"shipmentid":shipmentid}).then(res=>{
    if(res.data){
      state.isEdit=false;
