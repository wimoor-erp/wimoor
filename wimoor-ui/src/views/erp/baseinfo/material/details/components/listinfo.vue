...
<!--    </el-descriptions-item>-->
		...
		<el-descriptions-item label="店铺" v-if="type=='product'" :span="2">{{baseInfo.groupname}}</el-descriptions-item>
     ...
     <el-descriptions-item label="成本计算" :span="2">
       {{baseInfo.isAssPrice?'默认使用组装清单中的成本汇总':'使用自己的成本'}}
     </el-descriptions-item>
	 </el-descriptions>
	 <el-divider />
	 </div>
	...
	<el-descriptions  :column="1"  size="default" >
		...
		<el-descriptions-item label="采购单价（含税)" :span="2">
		...
		<el-descriptions-item label="增值税税率" :span="2">{{baseInfo.vatrate}}</el-descriptions-item>
		<el-descriptions-item label="退税率" :span="2">{{baseInfo.drawbackRate}}</el-descriptions-item>
	 ...
	 <el-descriptions  :column="1"  size="default" >
	  ...
	  <el-divider v-if="(logistics||baseInfo.addfee||custom)&&type=='product'" />
	  <div  v-if="type==='product'&&((logistics&&logistics.length>0)||baseInfo.addfee||custom)">
      ...
      <div v-if="custom&&custom.code" style="width:100%;padding-left:20px;padding-right:20px">
      <h4 id="logistics"  class="tab-scroll">报关-国内</h4>
      <table class="sd-table m-b-16" cellpadding="0" cellspacing="0" style="width:100%;margin-left:20px;margin-right:20px">
        <tr>
          <td class="table-label">海关编码(必填)</td><td>{{custom.code}}</td>
          <td class="table-label">中文名</td><td>{{custom.cname}}</td>
          <td class="table-label">英文名</td><td>{{custom.ename}}</td>
          <td class="table-label">报价</td><td>{{custom.price}}</td>
          </tr>
        <tr>
          <td class="table-label">材质(英文)</td><td>{{custom.material}}</td>
          <td class="table-label">材质(中文)</td><td>{{custom.materialcn}}</td>
          <td class="table-label">用途</td><td>{{custom.materialcn}}</td>
          <td class="table-label">关税</td><td>{{custom.rate}}</td>
        </tr>
        <tr>
          <td class="table-label">单位</td><td>{{custom.unit}}</td>
          <td class="table-label">子单位</td><td>{{custom.unit2}}</td>
          <td class="table-label">子单位比例</td><td>{{custom.unitrate}}</td>
        </tr>
        <tr>
          <td class="table-label">海关要素</td>
          <td colspan="7">{{custom.elements}}</td>
        </tr>
      </table>
      </div>
      <div  v-if="logistics&&logistics.length>0&&type==='product'" style="width:100%;padding-left:20px;padding-right:20px">
        <h4 id="logistics"  class="tab-scroll">报关-海外</h4>
        <el-space  wrap style="width:100%;padding-left:20px;padding-right:20px">
        <el-tag v-for="item in logistics" shadow="hover" style="width: 150px;"  :key="item.country">
          <span>{{item.country}}</span>: {{item.code}}
        </el-tag></el-space>
      </div>
      <!--
      <el-table  border :data="logistics">
          ...
          <el-table-column label="海关要素" prop="elements" >
          </el-table-column>
	   ...
	   <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
	      ...
	      <el-tab-pane v-if="type=='product'" label="FBA仓库" name="12">
			  ...
			  <el-table border :data="fbainventoryList">
					 ...
					 <el-table-column  prop="firstlegcharges" width="130">
                <template #header>
                  <div>头程运费(元/个)</div>
                </template>
							  <template #default="scope">
