...
function selectChange(selection) {
   ...
   function updateCycle(row){
	   if(!row.oldstockingCycle && row.oldstockingCycle !== 0){
		   ElMessage.warning("请输入安全库存");
		   return;
	   }
	   var list=[{"materialid":row.id,"warehouseid":state.queryParams.warehouseid,
	   "stockingcycle":parseInt(row.oldstockingCycle)}];
