<template>
	<el-row>
	   ...
	   <el-input  v-model="searchKeywords" clearable @input="changeData" placeholder="请输入" class="input-with-select" >
        ...
        <el-select v-model="isdelete"  @change="submitQuery()" style="width:80px" >
          <el-option label="启用"  key="0" value="0"></el-option>
          <el-option label="停用"   key="1" value="1"></el-option>
        </el-select>
	  ...
	  <el-row>
	   <el-space >
		...
		<el-dropdown trigger="click" v-hasPerm="'erp:material:download'"  >
          ...
          <el-dropdown-item @click="downloadProduct('moresupplier')" >导出备选供应商</el-dropdown-item>
	   ...
	   <div class='rt-btn-group'>
	    ...
	    <Helper name="产品管理" v-if="type=='product'" />
       <Helper name="辅料管理" v-if="type=='consumable'" />
       <Helper name="箱子管理" v-if="type=='package'" />
	   </div>
	</el-row>
	<!-- 上传文件弹框 -->
	<el-dialog
    ...
    if(state.selectRows && state.selectRows.length>0){
      ...
      state.selectRows.forEach(function(item){
        if(item.isDelete==true || item.isDelete==1){
          recoverList.push(item);
        }
      });
      if(recoverList.length>0){
        var successCount=0;
        var failCount=0;
        for(var i=0;i<recoverList.length;i++){
          try{
            var res=await materialApi.recoverData({"id":recoverList[i].id,"sku":recoverList[i].sku});
            if(res.data){
              successCount++;
            }else{
              failCount++;
        ...
        if(successCount>0){
          ElMessage.success('批量启用成功，成功'+successCount+'条'+(failCount>0?'，失败'+failCount+'条':''));
          changeData();
        }else if(failCount>0){
