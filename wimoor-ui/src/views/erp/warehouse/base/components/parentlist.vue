 <template>
    <!-- 搜索表单 -->
		  <div class="flex-center-between m-b-16">
			  <h5>
          <el-tooltip content="仓库分组名称(实际的仓库位置，系统库存单位是仓位，仓库类似于仓库地址)" placement="top">
            仓库分组
          </el-tooltip>
          </h5>
    ...
    >
      <el-form
        ...
        label-width="120px"
      >
        <el-form-item label="仓库分组名称" prop="name">
          <el-input v-model="formData.name" placeholder="仓库分组名称" />
        </el-form-item>
		<el-form-item label="分组次序" prop="findex">
		  <el-input v-model="formData.findex" placeholder="仓库分组次序" />
		</el-form-item>
...
import warehouseApi from "@/api/erp/warehouse/warehouseApi"
	...
	const state = reactive({
	  ...
	  rules: {
		name: [{ required: true, message: '请输入仓库组名称', trigger: 'blur' }],
	  },
	});
