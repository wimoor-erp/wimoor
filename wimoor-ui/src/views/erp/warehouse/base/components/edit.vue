	...
	<el-form ref="dataFormRef" :model="formData"
	 ...
	 label-width="120px">
		<el-form-item label="仓库">
		  <el-select v-model="formData.parentid" :disabled="formData.disabled" @change="warehouseChange" placeholder="请选择仓位地址">
	    ...
	    <el-form-item label="仓位名称"  prop="name">
	      <el-input v-model="formData.name" :placeholder="'建议包含仓库分组名称 '+formData.warehouseName+'- 作为前缀'" :disabled="formData.disabled"/>
	    </el-form-item>
		<el-form-item v-if="props.ftype!='oversea'" label="地址">
		 ...
		 <div class="font-extraSmall">(仅用于库位绑定,建议地址名称等于分组名称)</div>
		 </el-space>
		</el-form-item>
...
<script setup>
	...
	function warehouseChange(){
       formData.value.warehouseName=warehouseOptions.value.find(item=>item.id==formData.value.parentid).name;
  }
