...
<div class="el-white-bg">
						 ...
						 <h3>盘点单详情</h3>
               <el-space>
                 <PrinterBtn printId="stocktakePrintArea" :fitToPage="true" title="盘点单详情" />
                 <Helper name="库存盘点" />
               </el-space>

						 </div>
						 <el-scrollbar style="height:calc(100vh - 200px);" class="fill-from-body" id="stocktakePrintArea">
						 <el-form :model="form"
						   :rules="rules"
						   label-width="110px">
							  <el-row>
							 ...
							 <el-col :span="12">
								 <el-form-item label="类型" prop="warehouse">
									 ...
									 <div v-if="form.ftype==3">海外仓位</div>
								 </el-form-item>
							 </el-col>
							 <el-col :span="12" >
						 ...
						 <el-tabs v-model="activeWarehouse" style="width:100%;padding-top:20px" class="table-tabs" @tab-change="loadItemList(activeWarehouse)">
						     <el-tab-pane v-for="item in warehouseList" :label="item.name" :name="item.id" > </el-tab-pane>
						   </el-tabs>
						 <div class="mark-re" style="width:100%">
						   <h5 >产品列表</h5>
						 </div>
						 ...
						 <el-row :gutter="16" class="m-t-16  " style="width:100%;padding-bottom:20px">
							 <el-col :span="24" >
						...
						<div class='text-center mar-top-16'>
							 <div style="padding-top:10px;">
								<el-space>
									...
									<el-button v-if="form.isworking==true" style="margin-left: 10px;"  type="primary" @click.stop="continueHandle">继续盘点</el-button>
								 <el-button v-if="form.isworking==true" style="margin-left: 10px;"  type="danger" @click="cancelForm">取消盘点</el-button>
								</el-space>
							</div>
						</div>
	</div>
...
<script setup>
  ...
  import Helper from '@/components/header/helper.vue';
	...
	function load(){
		stocktakingApi.view({"id":id}).then((res)=>{
			if(res.data){
				...
				if(state.form.ftype==3&&res.data.warehouselist&&res.data.warehouselist.length>0){
						 res.data.warehouselist.forEach(item=>{
							checkid.push(item.warehouseid) ;
						 })
				}
	...
	function loadItemList(itemid){
		...
		if(state.form.ftype==1||state.form.ftype==3){
