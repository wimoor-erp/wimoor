<template>
	<div class="el-white-bg">
						 ...
						 <el-scrollbar style="height:calc(100vh - 200px);" class="fill-from-body">
						  ...
						  <el-col :span="12">
							 ...
							 <el-col v-if="ftype==1||ftype==3" :span="8">
								 <el-form-item label="仓位"   prop="warehouse">
									<el-select v-model="warehouseCheck" v-if="ftype==3"  @change="warehouseChange" placeholder="请选择仓位" :disabled="iswork">
						...
						<el-tabs v-model="activeWarehouse"   class="table-tabs" @tab-change="loadItemList()">
						 ...
						 <GlobalTable   ref="globalTable" :tableData="tableData" 
							border @loadTable="loadTableData" >
							 ...
							 <el-table-column  v-if="ftype==1||ftype==3"  label="可用库存" prop="fulfillable" sortable="custom"/>
							 ...
							 <el-table-column label="盘点数量">
							  ...
							  <el-table-column v-if="ftype==1||ftype==3" label="账面数量"  >
								  <template #default="scope">
...
<script setup>
 ...
 function saveItem(row){
	 if(state.ftype==1||state.ftype==3){
	...
	function loadTabsData(){
		...
		console.log(state.warehouseCheck);
		if(tabsid!=[] && tabsid!=null && tabsid!=""){
			if(ftype==1){
				...
				stocktakingApi.stocktakingListShelf(tabsid).then(res=>{
					...
					state.warehouseList = state.warehouseData
					state.activeWarehouse =state.warehouseCheck;
					loadItemList();
			}
		}
	}
function warehouseChange(val) {
	console.log(val);
        ...
        var warehouseid =[];
				val.forEach((values)=>{
					...
					state.warehouseCheck=val;
			}
			loadTabsData();
		} else {
			if(state.ftype==1){
				state.warehouseCheck=[];
			}else if(state.ftype==2){
				state.warehouseCheck=[];
			} else {
				state.warehouseCheck="";
	...
	function loadWarehouse(){
		if(state.ftype==1){
			...
			warehouseApi.getOversaWarehouseUseable().then(function(res){
				state.warehouseData = res.data;
			})
		}
	}
	function load(){
			stocktakingApi.edit({"id":id}).then((res)=>{
				if (res.data) {
					...
					if(state.ftype==3&&res.data.warehouselist&&res.data.warehouselist.length>0){
						 res.data.warehouselist.forEach(item=>{
							checkid.push(item.warehouseid) ;
						 })
						  state.warehouseCheck=checkid[0];
					 }
					 loadTabsData();
					 loadWarehouse();
				}
