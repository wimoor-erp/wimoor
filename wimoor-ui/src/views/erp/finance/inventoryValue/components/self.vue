<template>
	...
	<el-input  v-model="queryParams.sku" :clearable="true" placeholder="请输入本地SKU" @input="handleQuery" class="input-with-select" >
	  ...
	  <el-space>
	    <el-checkbox v-model="queryParams.isAvgPrice" @change="handleQuery">使用采购加权平均成本</el-checkbox>
	  <Helper name="库存货值" />
	</el-space>	
 </div>
...
<script setup>
	...
	const state=reactive({
		...
		summary:{},
		queryParams:{sku:"",byday:new Date().format("yyyy-MM-dd"),today:true,isAvgPrice:false}
	})
	const{
