<template>
	...
	<el-table 
	 ...
	 size="small">
		...
		<el-table-column label="备货期内日均销量" min-width="150" header-align="left" align="left" >
			<template #default="scope">
				<div style="display:inline-flex;align-items:center;gap:4px;cursor:pointer;" @click="handleAdjustSale(scope.row)">
				<div style="width:50px;" v-if="scope.row.avgsales">{{scope.row.avgsales}}
				...
				<div v-if="hasPrelistData(scope.row)"
				     ...
				     style="width:80px;height:20px;flex-shrink:0;overflow:visible;">
				</div>
				</div>
			</template>
		</el-table-column>
		<el-table-column label="备货周期" header-align="left" align="left" >
			<template #default="scope">
		...
		<el-table-column label="可销售天数" header-align="center" align="center" >
          ...
          <el-tooltip v-if="scope.row.salesdayAvgsales"
                 ...
                 content="可销售天数内日均销量"
				      >
				      <span v-if="scope.row.salesdayAvgsales>0 && scope.row.salesdayAvgsales!==scope.row.avgsales" class="font-extraSmall" >({{scope.row.salesdayAvgsales}})</span>
				      </el-tooltip>
...
<script setup>
 ...
 function handleRowClassName({row,rowindex}){
	...
	// prelist 微型折线图 - 轻量SVG实现（带缓存避免重复渲染）
const sparklineCache = new Map();
function hasPrelistData(row) {
	if (!row.prelist || typeof row.prelist !== 'object') return false;
	const entries = Object.entries(row.prelist);
	if (entries.length < 2) return false;
	const quantities = entries.map(([, item]) => item.quantity || 0);
	return !quantities.every(q => q === quantities[0]);
}
function initSparkline(el, row) {
	if (!el) return;
	const key = row.sku + '_' + row.marketplaceid;
	if (sparklineCache.has(key)) {
		el.innerHTML = sparklineCache.get(key);
		return;
	}
	nextTick(() => {
		drawSparkline(el, row);
	});
}
function drawSparkline(el, row) {
	if (!row.prelist) return;
	const entries = Object.entries(row.prelist)
		.map(([date, item]) => ({ date, quantity: item.quantity || 0 }))
		.sort((a, b) => a.date.localeCompare(b.date));
	if (entries.length === 0) return;
	const W = 80, H = 20, PAD = 2;
	const vals = entries.map(e => e.quantity);
	const minV = Math.min(...vals), maxV = Math.max(...vals);
	const range = maxV - minV || 1;
	const points = entries.map((e, i) => {
		const x = PAD + (i / (entries.length - 1)) * (W - 2 * PAD);
		const y = PAD + (1 - (e.quantity - minV) / range) * (H - 2 * PAD);
		return `${x.toFixed(1)},${y.toFixed(1)}`;
	});
	const polyline = points.join(' ');
	const areaPoints = `${PAD},${H - PAD} ${polyline} ${W - PAD},${H - PAD}`;
	// 只在quantity发生变化的拐点显示完整日期和数量，连续相同值显示省略号
	const tipParts = [];
	let prevQ = null;
	for (let i = 0; i < entries.length; i++) {
		if (entries[i].quantity !== prevQ) {
			tipParts.push(`${entries[i].date}: ${entries[i].quantity}`);
			prevQ = entries[i].quantity;
		} else {
			if (i === entries.length - 1 || entries[i + 1].quantity !== prevQ) {
				tipParts.push(`...: ${entries[i].quantity}`);
			}
		}
	}
	const tipLines = tipParts.join('\n');
	const svgHtml = `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="display:block;">
		<defs><linearGradient id="g_${row.sku}" x1="0" y1="0" x2="0" y2="1">
			<stop offset="0%" stop-color="rgba(255,116,22,0.25)"/>
			<stop offset="100%" stop-color="rgba(255,116,22,0.02)"/>
		</linearGradient></defs>
		<polygon points="${areaPoints}" fill="url(#g_${row.sku})"/>
		<polyline points="${polyline}" fill="none" stroke="#ff7416" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>
		<title>${tipLines}</title>
	</svg>`;
	sparklineCache.set(row.sku + '_' + row.marketplaceid, svgHtml);
