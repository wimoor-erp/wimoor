<template>
	...
	<div v-if="ftype=='return'">
        ...
        <el-button v-if="ftype=='delete'" type="danger" @click="handleDelete">作废</el-button>
		</template>
...
<script setup>
  ...
  function handleDelete(){
          purchaselistApi.deleteEntry({"id":state.nowrow.id}).then((res)=>{
            if(res.data){
              state.nowrow.auditstatus=0;
              state.dialogVisable=false;
