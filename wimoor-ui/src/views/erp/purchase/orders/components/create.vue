					...
					<div v-loading="tableLoading">
						 <table class="sd-table"  border="0" cellpadding="0" cellspacing="0">
							 ...
							 <tbody v-for="(item,index) in tableData">
										 ...
										 <el-table :data="item.subsku" size="small">
											 ...
											 <el-table-column label="剩余库存" width="200" prop="inbound" >
													<template #default="scope">
														...
														<div v-else>0</div>
														<div class="font-extraSmall" v-if="(scope.row.submap && scope.row.submap.newmoreqty != ((scope.row.submap.amount || 0) + (scope.row.submap.inbound || 0) - (scope.row.submap.needprocess || 0)))">
                              ...
                              <span  class="text-red" v-if="(scope.row.submap&&scope.row.submap.inbound ? scope.row.submap.inbound : 0) + (scope.row.submap&&scope.row.submap.needprocess ? scope.row.submap.needprocess : 0) !== 0">({{(scope.row.submap&&scope.row.submap.amount ? scope.row.submap.amount : 0) +
                              (scope.row.submap&&scope.row.submap.inbound ? scope.row.submap.inbound : 0) -
                              (scope.row.submap&&scope.row.submap.needprocess ? scope.row.submap.needprocess : 0)}})
                             </span>
													</div>
