			  ...
			  <!-- 空格间距-->
			    <tr class="sep-row"><td :colspan="tableState=='4' || tableState=='5' || tableState=='6'? 10:8"></td></tr>
                  ...
                  <span v-if="item.groupname">
									<div class="flex-center text-gray">
                    <el-icon class="font-base "><Shop /></el-icon>
								  <span class="m-t-r-2-4">  {{item.groupname}}</span>
								  </div>
								  </span>
						 ...
						 <el-space class="m-t-8">
							 <el-button size="small" link class="text-gray"   @click="downloadTemplateExcel(item)">
...
<!--               </el-button>-->
  ...
  <TemplateDialog ref="templateDialogRef"  @download="handleDownload" ></TemplateDialog>
</template>
...
<script setup>
  ...
  import TemplateDialog from "@/components/template/index.vue";
	import {updateItem,handleChangesub,changeItemPrice,changeItemOrderprice,
	        ...
	        updateCycleDate,reSubmit} from  '@/hooks/erp/purchase/form.js';
 ...
 function getRemarks(e,row,remarktypename){
  ...
  function handleDelete(row){ 	//单据作废
    if(row.paystatus==3){
      ElMessage.error( '单据正在请款中，暂不支持作废！')
    }else{
      AssemblyRef.value.show(row,"delete");
    }
  }
  ...
  function handleDownload(templateid,row){
    var data={};
    ...
    data.formid=row.id;
    purchaselistApi.downloadPurchaseInfoData(data).then((res)=>{
      templateDialogRef.value.download(templateid,res.data);
    });
  }
