<template>
	...
	<div class="con-header" style="padding: 16px 16px 0px 16px;">
      ...
      <Helper name="FBA发货规划" style="margin-left:10px;"/>
		</el-row>
		<el-row>
			<el-space >
				...
				<el-button @click="showInventoryReportTask" >库存报告任务</el-button>
				</el-space >
	...
	<TaskDataDialog ref ="taskDataDialogRef"></TaskDataDialog>
...
<script setup>
	...
	import { ref ,reactive,onMounted,onUnmounted,toRefs,inject} from 'vue'
	...
	import TaskDataDialog from "@/views/amazon/storeAuth/components/taskData.vue";
	// API依赖
	   ...
	   function clearPlan(){
		...
		onMounted(()=>{
			...
			timer=setInterval(function(){
				...
				affixScrollHandler = ()=>{
					affixReady.value = window.scrollY > 170
				}
				window.addEventListener('scroll', affixScrollHandler)
		});
onUnmounted(()=>{
    if(affixScrollHandler){
        window.removeEventListener('scroll', affixScrollHandler)
    }
})
			...
			function showInventoryReportTask(){
				if(state.queryParams.groupid){
					authApi.getBindSeller().then(res=>{
						if(res.data&&res.data.length>0){
							let groupData=res.data.find(item=>item.groupid==state.queryParams.groupid);
							if(groupData&&groupData.regions){
								let allCountrysList=[];
								let authid=null;
								let sellerid=null;
								Object.values(groupData.regions).forEach(region=>{
									if(region.countrysList){
										allCountrysList=allCountrysList.concat(region.countrysList);
									}
									if(region.authid){
										authid=region.authid;
									}
									if(region.sellerid){
								...
								let data={
									sellerid:sellerid||groupData.groupid,
									countrysList:allCountrysList,
									authid:authid,
                  groupid:groupData.groupid
								};
								taskDataDialogRef.value.show(data,"GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA");
							}else{
								ElMessage.error('未找到对应店铺信息');
							}
						}else{
