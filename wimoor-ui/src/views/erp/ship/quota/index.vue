...
<script setup>
	...
	const overseasformid=route.query.overseasformid;
	...
	function handleSubmit(){
		   ...
		   }else if(overseasformid){
		    dispatchApi.checkinv(state.formid, overseasformid).then(res=>{ getshipFormData(); })
		   }
	}
	const getshipFormData = function(){
		...
		if(shipmentid){
			...
			console.log('overseasformid:', overseasformid);
			dispatchApi.getQuotainfo(overseasformid).then((res)=>{
				console.log('getQuotainfo response:', res);
				state.loading=false;
				state.orderData = res.data
				state.activities[0].content = state.orderData.warehouse
				state.activities[1].content = state.orderData.groupname+'-'+state.orderData.country+'('+state.orderData.center+')'
				...
				console.error('getQuotainfo error:', error);
				state.loading=false;
			});
		}else if(assemblys){
