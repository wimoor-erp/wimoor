<template>
  <div >
    ...
    <el-dialog :title="title" v-model="open" width="80%" append-to-body>
      <el-form ref="typesRef" :model="form" label-width="auto" :rules="rules" >
        <el-form-item label="类型名称" prop="typeName">
          <el-input v-model="form.typeName" placeholder="请输入类型名称：部门、员工、客户、项目、合同、供应商、银行账户、成本中心、现金流量项目等" />
        </el-form-item>
        <el-form-item label="类型编码" prop="typeCode">
          <el-input v-model="form.typeCode" placeholder="请输入类型编码：DEPT、EMPLOYEE、CUSTOMER、PROJECT、CONTRACT、SUPPLIER、BANK_ACCOUNT、COST_CENTER、CASH_FLOW_ITEM等" />
        </el-form-item>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          ...
          <el-select v-model="queryParams.status" @change="getList"  placeholder="请选择状态" clearable>
            <el-option
                v-for="dict in subject_status"
                :key="dict.value"
                :label="dict.label"
      ...
      <el-table v-loading="loading" :data="typesTable" @selection-change="handleSelectionChange">
...
const { queryParams,typesTable, form, rules, type } = toRefs(data)
...
async function getList() {
  ...
  listTypes(queryParams.value).then(response => {
    ...
    data.typesTable=response.rows;
    if (typesList.value.length > 0) {
      type.value = typesList.value[0].typeId;
      emit('change', { typeId: type.value, typeCode: typesList.value[0].typeCode });
    }
...
function handleChange(){
  const selectedType = typesList.value.find(item => item.typeId === type.value);
  emit('change', { typeId: type.value, typeCode: selectedType ? selectedType.typeCode : null });
}
// 多选框选中数据
