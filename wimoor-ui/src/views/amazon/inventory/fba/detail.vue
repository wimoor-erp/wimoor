<template>
	<div class="main-sty">
		...
		<el-input  v-model="queryParams.search" @input="handleQuery" clearable placeholder="请输入SKU" style="width: 250px;" class="input-with-select" >
		 ...
		 <el-button type="primary" @click.stop="uploadExcel">头程费用导入</el-button>
       ...
       <GlobalTable ref="globalTable"
         ...
         <template #field>
           ...
           <el-table-column label="安全库存周期" prop="stockingCycle"  width="120" sortable="custom"  />
           <el-table-column label="最小发货周期" prop="minCycle"  width="120" sortable="custom"  />
           <el-table-column label="头程运输成本" prop="firstLegCharges"  width="120" sortable="custom"  />
         </template>
       </GlobalTable>
...
<script setup>
  ...
  let uploadDialogRef=ref();
	...
	let uploadRptRef=ref(); // 确保ref已经定义
	...
	let {
		...
		summary,radiotype,
		 ...
		 function uploadExcel(){
			 // 调用UploadDialog组件的show方法
       uploadDialogRef.value.show({'templateCallback':downloadExcel,'title':"产品头程费用导入"});
		 }

  function handleUpload(formDatas){
    inventoryRptApi.uploadInventoryFile(formDatas).then(res=>{
      ...
      uploadDialogRef.value.hide();
		 ...
		 function loadTableData(params){
			 inventoryRptApi.getWarehouse(params).then(res=>{
				 ...
				 if(params.currentpage==1){
					 if(res.data.total>0){
             if(res.data.records[0] && res.data.records[0].summary){
		 ...
		 function getSummaries(){
		 	var arr = ["合计"]
       if(state.summary && state.summary.afnWarehouseQuantity){
