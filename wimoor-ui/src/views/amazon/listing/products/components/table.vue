		 ...
		 <el-table-column label="产品信息" min-width="400" >
					...
					<el-space :size="8" class=" icon-text-center">
						...
						<el-space class="font-extraSmall" >
							...
							<div  class="pointer" >
							     <img  :src="$require('ai.png')" 
								        @click.stop="handleShowAI(scope.row)"
							           :title="'本地SKU:'+scope.row.msku" 
...
<el-row v-else-if="activeName=='third'">
	 ...
	 <!-- AI对话弹框 -->
 <ProductAIDialog ref="productAIRef"></ProductAIDialog>
 </div>
	 <el-popover
	 placement="bottom"
...
<script>
  ...
  import ProductAIDialog from "@/views/amazon/listing/products/components/productAIDialog.vue";
	export default {
		name:'Table',
		components: {
			...
			CaretTop,ChartLine,ArrivalDialog,OtherCost,OwnerAll,FollowDialog,DownTwo,UpTwo,ProductPlus,ProductAIDialog,
		},
	...
	//获取父asin列表	
		 ...
		 function handleShowAI(row){
       productAIRef.value.show(row);
			context.emit("changeAI",row);
		 }
		...
		function objectSpanMethod({ row, column, rowIndex, columnIndex }) {
		 ...
		 function handleShowFeedback(row){
			...
			handleShowFeedback,handleShowAI,
			//variable
			currentRowData,
			//ref
			salechartRef,matchingRef,modifypriceRef,remarksRef,followDialogRef,fbaInvHistoryDialogRef,feedbackDialogRef,
			porfitRef,arrivalchartRef,statusRef,otherCostRef,ownerRef,advShowHideRef,RecommendedAction,productPlusRef,productAIRef
			}
		},
	}
</script>
