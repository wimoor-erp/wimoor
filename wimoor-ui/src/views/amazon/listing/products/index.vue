	...
	<div class="main-sty" >
		<div class="con-header">
          ...
          <el-button @click.stop="showSyncDialog">批量同步商品</el-button>
					<el-button @click.stop="downloadMaterialInfo"  :loading="downloadMaterialLoading">批量下载本地SKU信息</el-button>
				</el-space>
...
<script setup>
  ...
  import {percentInput, CheckInputFloat, decodeText} from '@/utils/index.js';
  ...
  import store from '@/store/index.js';
      ...
      let downloadMaterialLoading=ref(false);
			let groupRef=ref();
			let tableSkuRef=ref();
			let ownerRef=ref();
			let statusRef=ref();
    ...
    function refreshSync(){
      ...
      initProductTask(syncForm).then((res)=>{
		...
		// 批量下载本地SKU信息
		async function downloadMaterialInfo() {
      var data={};
      downloadMaterialLoading.value = true;
      data.groupid=queryParam.groupid;
      data.taglist=queryParam.taglist;
      data.marketplace=queryParam.marketplaceid;
      data.search=queryParam.search;
      data.searchtype=queryParam.searchtype;
      data.isfba=queryParam.isfba;
      data.ownerid=queryParam.owner;
      data.isparent=queryParam.isparent;
      if(queryParam.salestatus===""){
        queryParam.salestatus="all";
      }
      data.salestatus=queryParam.salestatus;
      data.disable=queryParam.disable;
      data.changeRate=queryParam.changeRate;
      data.remark=queryParam.remark;
      data.category=queryParam.category;
      data.isbadreview=queryParam.isbadreview;
      data.name=queryParam.name;
      data.paralist=queryParam.paralist;
      data.downloadMSku=true;
      if(queryParam.sort){
        data.sort=queryParam.sort;
        data.order=queryParam.order;
      }
      if(queryParam.marketplace){
        data.marketplace=queryParam.marketplace;
      }
      data.pagesize=2000;
      data.currentpage=1;
      productinfoApi.productList(data).then((res)=>{
        if(res.data){
          ...
          // 遍历数据
          for (let row of res.data.records) {
            // 只添加不重复的 msku
            if (row.msku && !mskuSet.has(row.msku)) {
              mskuSet.add(row.msku);
            }
          }
            // 将 Set 转换为逗号分隔的字符串
          sku = Array.from(mskuSet).join(',');
          var data={};
          let  ftype='MaterialBaseInfo';
          data.downtype=ftype;
          data.mtype="product";
          data.searchlist=sku;
          materialApi.downExcelRecords(data,()=>{ downloadMaterialLoading.value = false;});
        }else{
          tableData.records = [];
          tableData.total =0;
        }
