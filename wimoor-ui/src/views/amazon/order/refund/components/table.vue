			...
			<GlobalTable  ref="globalTable" :tableData="tableData"   height="calc(100vh - 256px)" @loadTable="loadTableData" :stripe="true"  style="width: 100%;margin-bottom:16px;">
				 <template #field>
				  <el-table-column prop="groupname"  label="站点" width="100"  sortable="custom" >
				   ...
				   <el-table-column prop="orderId" label="订单号" sort-by="order_id" width='180' sortable="custom" />
				...
				<el-table-column prop="purchaseDate" label="购买时间" width="100"  sortable="custom" >
					<template #default="scope">
					    {{dateFormat(scope.row.purchaseDate)}} 
					</template>
				</el-table-column>	
			    <el-table-column prop="returnDate"  sort-by="return_date" label="退款时间" width="100"  sortable="custom" >
					<template #default="scope">
				...
				<el-table-column prop="detailedDispositionName" label="商品状况" width="120" sort-by="detailed_disposition" sortable="custom"   >
					<template #default="scope">
					   <div> {{scope.row.detailedDispositionName}} </div>
					   <div class="font-extraSmall"> {{scope.row.statusname}} </div>
					</template>
				</el-table-column>
				<el-table-column prop="reasonname" label="退货原因"  sort-by="reason"   sortable="custom"/>
				<el-table-column prop="customerComments" label="退货留言"  sort-by="customer_comments"   sortable="custom"/>
				<el-table-column prop="licensePlateNumber" label="追踪编号"  sort-by="license_plate_number"   sortable="custom"/>
			  </template>
			 </GlobalTable>
			</el-row>
</template>
