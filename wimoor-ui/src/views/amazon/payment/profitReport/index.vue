		...
		<div class="main-sty con-header"> 
			...
			<!-- 	 <Group ref="groups" @change="getData" defaultValue="all"/> -->
			  ...
			  <div class='rt-btn-group'>
               ...
               <el-button @click="downloadListOld" :loading="loading">导出（旧）</el-button>
			  				   <el-button
			  				    @click="recalculation"
			  				    > 重算 </el-button>
			   ...
			   <!--功能区域 -->
			<TableHorizontal  :activeName="queryParams.charttype" @changeTable="handleChnageTable" ref="tableRef"/>
    ...
    function downloadListOld(){
      state.loading=true;
      settlementMonthSummaryApi.downloadListOld(state.queryParams).then((res)=>{
        state.loading=false;
      }).catch(e=>{
        state.loading=false;
      });
