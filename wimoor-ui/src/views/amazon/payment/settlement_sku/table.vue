			...
			<el-space :size="4">
			  <span>毛利润</span>
			   <el-tooltip
			        ...
			        >
					 <template #content> 
					 <div class="font-bold">计算公式：</div>
					 <div>• 毛利润 = SKU结算 - 本地成本预估 - 采购成本 - 自定义费用 + 退税</div>
					 <div>• 本地成本预估 = 预估运费 + 预估市场费用 + 预估VAT + 预估所得税 + 预估关税 + 预估汇率损耗 + 预估固定费用 + 预估其他费用</div>
					 
					 <div class="font-bold mt-2">费用组成：</div>
					 
					 <div>• 平台费用：含FBA费用、销售佣金、买家运费、促销费、其它收支等</div>
					 <div class="font-bold mt-2">说明：</div>
					 <div>• 所有平台费用与分摊费用，减项自动显示为负数</div>
					 <div>• 预估费用基于系统设置的规则计算得出</div>
					  </template>
