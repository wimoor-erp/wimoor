	...
	<div>
		<div class="con-header">
			...
			<el-row>
					...
					<el-table :show-header="false" :data="item.regionslist" border  :span-method="rowmerge">
						...
						<el-table-column  prop="sellerauth">
							<template #default="scope">
								...
								<div>
								    <el-button  title="刷新站点" v-if="scope.row.authid" link type="primary" :loading="scope.row.authloading" size="small" @click="refreshAuth(scope.row,scope.row.authid)">刷新</el-button>
									<div style="margin-top:10px;">
									<el-button link type="primary"  @click="showTaskDialog(scope.row)">报表进度</el-button>
									</div>
								 </div>
								 </el-space>
                <div class="font-extraSmall"><span class="font-extraSmall">过期日期：</span>

                {{dateFormat(getExpireTime(scope.row.time))}}
                </div>
							</template>	
						</el-table-column>
						<el-table-column  prop="adauth">
							<template #default="scope">
								...
								<div>
									<el-button   v-if="scope.row.advid"  title="刷新站点" :loading="scope.row.advloading" link type="primary" size="small" @click="refreshAdv(scope.row,scope.row.advid)">刷新</el-button>
								</div>	
								<div style="margin-top:10px;">
									<el-button  
                ...
                <div class="font-extraSmall"><span >过期日期：</span>
                  {{dateFormat(getExpireTime(scope.row.advtime))}}
                </div>
							</template>	
						</el-table-column>
...
function getExpireTime(date) {
  ...
  const nextYearDate = new Date(inputDate);
  nextYearDate.setFullYear(inputDate.getFullYear() + 1);
  return nextYearDate;
}
			function refreshAdv(row,id){
        row.advloading=true;
				authAdvApi.captureProfileById({"id":id}).then((res)=>{
          row.advloading=false;
          if(res.data!="error"){
            ElMessage.success("刷新成功！");
          }else{
            ElMessage.error("刷新失败请重试或重新授权！");
          }
				}).catch(e=>{
          row.advloading=false;
        });
			}
			function refreshAuth(row,id){
        row.authloading=true;
				authApi.refreshMarketByAuth({"authid":id}).then((res)=>{
					ElMessage.success("刷新成功！");
					row.authloading=false;
				}).catch(e=>{
          row.authloading=false;
        });
			}
