<template>
	...
	<div style="padding-bottom:10px ">
	      <el-space>
				   <div>重新申请报表：</div>
	      <el-select v-model="reportType" :disabled="lockedReportType" placeholder="请选择报表类型">
	      	 			 <el-option v-for="item in reportTypeList" :key="item.code" :label="item.cname" :value="item.code">{{item.cname}}</el-option>
	      </el-select>
	      <template v-if="lockedReportType">
	      		<el-tag type="success">已锁定报表类型：{{lockedReportTypeName}}</el-tag>
	      </template>
...
<script setup>
	...
	const  state=reactive({
		...
		reportTypeList:[],
		activeName:null,
		lockedReportType:false,
		lockedReportTypeName:"",
	})
	const{
		isload,
		tableData,
	 ...
	 function handleQuery(){
		 ...
		 if(state.lockedReportType && state.reportType){
			 state.queryParams.reportType=state.reportType;
       state.queryParams.groupid=state.groupData.groupid;
		 }else if(state.reportType){
			 state.queryParams.reportType=state.reportType;
		 }else{
			 delete state.queryParams.reportType;
		 }
	...
	function show(data, lockReportType){
		state.visible=true;
		state.lockedReportType=false;
		state.lockedReportTypeName="";
		state.reportType="";
		limitApi.reportTypeList().then((res)=>{
			state.reportTypeList=res.data;
			if(lockReportType){
				state.lockedReportType=true;
				state.reportType=lockReportType;
				const typeItem=res.data.find(item=>item.code===lockReportType);
				state.lockedReportTypeName=typeItem?typeItem.cname:lockReportType;
			}
    ...
    if(state.lockedReportType==true){
      let groupid=state.groupData.groupid;
      let marketplaceid=state.queryParams.marketplaceid;
      authApi.getAmazonAuthIdByGroupId({"groupid":groupid,"marketplaceid":marketplaceid}).then((res)=>{
        if(res.data){
          let authid=res.data;
          authApi.requestReportParamAuth({"authid":authid,
          ...
          ElMessage.error("获取店铺授权信息失败");
        }
      });
    }else{
      let authid=state.groupData.authid;
      authApi.requestReportParamAuth({"authid":authid,
                                     "type":state.reportType,
                                     "start":state.fromDate,
                                     "end":state.endDate,
