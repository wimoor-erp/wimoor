    ...
    const slotRef=ref();
	 ...
	 let router = useRouter();
	   ...
	   function tableSort(option){
        if(option.column.sortBy){
          state.mQueryParams.sort=option.column.sortBy;
        }else{
		...
		function loadTable(param){
			 ...
			 state.currentPage=1;
			 if(param){
			    state.mQueryParams=Object.assign({}, state.mQueryParams, JSON.parse(JSON.stringify(param)));
			 }
