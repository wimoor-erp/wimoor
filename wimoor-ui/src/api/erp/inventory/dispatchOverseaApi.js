...
function downExcelBoxDetail(data){
	return request({url:"/erp/api/v1/inventory/dispatch/oversea/downExcelRecords",
				                    responseType:"blob",
		                            params:data,
									method:'get'}).then(res => {
											downloadhandler.downloadSuccess(res,"boxdetail.xlsx")
									}).catch(e=>{
											downloadhandler.downloadFail();
									});
}
...
function getQuotainfo(id){
	return request.get('/erp/api/v1/inventory/dispatch/oversea/getQuotainfo', {params:{id:id}});
}

function checkinv(checkinv, overseasformids){
	return request.post('/erp/api/v1/inventory/dispatch/oversea/checkinv/'+checkinv, overseasformids);
}
...
export default{
	 ...
	 getQuotainfo,downExcelBoxDetail,
	 checkinv,
}