...
public class NotifyHandlerServiceImpl extends ServiceImpl<SystemNotifyMapper,SystemNotify> implements INotifyHandlerService {
	...
	public void read(UserInfo user, String notifyIDs) {
		String[] notifys = notifyIDs.split(",");
		for (String notifyid : notifys) {
	...
	public List<SystemNotify> findNoReadByUserAll(String userid) {
		List<SystemNotify> list = this.baseMapper.findNoReadByUserAll(userid);
		return list;
	}
