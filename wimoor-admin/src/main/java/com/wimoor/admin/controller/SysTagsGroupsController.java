...
import com.wimoor.admin.pojo.vo.SysTagsChildrenVo;
import com.wimoor.admin.pojo.vo.SysTagsVo;
import com.wimoor.admin.service.ISysTagsGroupsService;
...
public class SysTagsGroupsController {
    ...
    public Result<Boolean> delete(@PathVariable String ids) {
        ...
        List<String> codeList = iSysTagsGroupsService.listByIds(idList).stream().map(item -> item.getId()).collect(Collectors.toList());
        if (CollectionUtil.isNotEmpty(codeList)) {
            long count = iSysTagsService.count(new LambdaQueryWrapper<SysTags>().in(SysTags::getTaggroupid, codeList));
            Assert.isTrue(count == 0, "删除标签组失败，请先删除关联标签组数据");
        }
        boolean status = iSysTagsGroupsService.removeByIds(idList);
        return Result.judge(status);
    }
