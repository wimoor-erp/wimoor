...
public class AmzAdvRecommendationsTargetServiceImpl implements IAmzAdvRecommendationsTargetService{
	...
	public Map<String, Object> amzCreateTargetRecommendations(UserInfo user,BigInteger  profileId, List<String> asinsList,int pageSize, int pageNumber) {
		...
		String response = amzAdvAuthService.amzAdvPost(profile, url, param.toString());
	...
	public List<Map<String, Object>> amzTargetRecommendationsProductList_SD(UserInfo user, BigInteger profileId, List<String> paramList, int maxResults, String nextToken) {
		...
		JSONArray typeFilterjson = new JSONArray();
		for(String asin : paramList) {
			JSONObject asinjson = new JSONObject();
			String[] asins = asin.split(",");
			for(int i = 0; i < asins.length; i++) {
				asinjson.put("asin", asins[i]);
			}
			productsjson.add(asinjson);
		}
		typeFilterjson.add("PRODUCT");
		param.put("tactic", "T00020");
		param.put("products", productsjson);
		param.put("typeFilter", typeFilterjson);
		String url = "/" + AmzAdvCampaignServiceImpl.CampaignType.sd + "/targets/recommendations";
		AmzAdvProfile profile = amzAdvAuthService.getAmzAdvProfileByKey(profileId);
		String contentType = "application/vnd.sdtargetingrecommendations.v3.0+json";;
		String response = amzAdvAuthService.amzAdvPost_V3(profile, url, param.toString(),contentType);
		if (StringUtil.isNotEmpty(response)) {
			JSONObject jsonObect = GeneralUtil.getJsonObject(response.toString());
			  JSONObject json= jsonObect.getJSONObject("recommendations");
			  JSONArray jsonArray = json.getJSONArray("products");
			List<Map<String, Object>> list = new ArrayList<Map<String,Object>>();
			for (int i = 0; i < jsonArray.size(); i++) {
				JSONObject jsonItem = jsonArray.getJSONObject(i);
				Map<String,Object> map=new HashMap<String,Object>();
				for(Entry<String, Object> item:jsonItem.entrySet()) {
