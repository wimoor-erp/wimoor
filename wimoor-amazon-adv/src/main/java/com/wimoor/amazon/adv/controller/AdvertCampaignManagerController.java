...
//		request.setAttribute("groupList", advGroupList);
	...
	public Result<PageList<Map<String, Object>>> getCampaignListAction(@ApiParam("查询广告活动") QueryForList query) {
		UserInfo user = UserInfoContext.get();
	...
	public Result<Object> getgetCampaignDetailAction(@ApiParam("查询广告活动") QueryForList query) {
		UserInfo user = UserInfoContext.get();
		Map<String, Object> map = AdvertController.amzAdvParameterMap(query);
		map.put("shopid", user.getCompanyid());
