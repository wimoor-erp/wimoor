...
//	AmzProductActiveDayNumMapper amzProductActiveDayNumMapper;
		...
		if (list != null && list.size() > 0) {
			...
			if (sumclicks != null && sumclicks != 0) {
				map.put("cr", new BigDecimal(sumOrdered).multiply(new BigDecimal("100"))
						.divide(new BigDecimal(sumclicks), BigDecimal.ROUND_HALF_UP, 2));
			} else {
				map.put("cr", 0);
			}
			...
			if (sumSales != null && sumSales.compareTo(new BigDecimal("0")) != 0) {
				map.put("acos", sumcost.multiply(new BigDecimal("100")).divide(sumSales, BigDecimal.ROUND_HALF_UP, 2));
			} else {
				map.put("acos", 0);
			}
		} else {
	...
	public Map<String, Object> getDaysSumProduct(Map<String, Object> param) { 
		...
		for (Entry<String, ArrayList<Map<String, Object>>> itemEntry : mapList.entrySet()) {
			...
			if (list != null && list.size() > 0) {
				...
				costMap.put(newkey, sumcost.setScale(2, BigDecimal.ROUND_UP));
				orderMap.put(newkey, sumOrdered);
				attributedSalesMap.put(newkey, sumSales.setScale(2, BigDecimal.ROUND_UP));
				if (!(sumSales.compareTo(BigDecimal.ZERO) == 0)) {
					sumAcos = sumcost.divide(sumSales,2, BigDecimal.ROUND_HALF_UP);
				} 
	...
	public Map<String, Object> getMonthSumProduct(Map<String, Object> param) {
		...
		for (Entry<String, ArrayList<Map<String, Object>>> itemEntry : mapList.entrySet()) {
			...
			if (list != null && list.size() > 0) {
				...
				if (sumclicks != 0) {
					...
					cpc.put(newkey, sumcost.divide(new BigDecimal(sumclicks), BigDecimal.ROUND_HALF_UP, 2));
				} else {
					cr.put(newkey, 0);
				}
				if (sumSales.compareTo(new BigDecimal("0")) != 0) {
					acos.put(newkey, sumcost.multiply(new BigDecimal("100")).divide(sumSales, BigDecimal.ROUND_HALF_UP, 2));
				} else {
					acos.put(newkey, 0);
				}
		...
		if (sumAllClicks != 0) {
			...
			resultMap.put("sumAllcpc", sumAllCost.multiply(new BigDecimal("100")).divide(new BigDecimal(sumAllClicks), BigDecimal.ROUND_HALF_UP, 2));
		} else {
			resultMap.put("sumAllcr", 0);
		}
		if (sumAllSales.compareTo(new BigDecimal("0")) != 0) {
			resultMap.put("sumAllacos", sumAllCost.multiply(new BigDecimal("100")).divide(sumAllSales, BigDecimal.ROUND_HALF_UP, 2));
		} else {
		...
		if (allsale != null && allsale.containsKey("listLabel")) {
			...
			for (int i = 0; i < lebels.size(); i++) {
				...
				if (all.compareTo(new BigDecimal("0")) != 0) {
					acoaslist.add(cost.multiply(new BigDecimal("100")).divide(all, BigDecimal.ROUND_HALF_UP, 2));
					salepalllist.add(sale.multiply(new BigDecimal("100")).divide(all, BigDecimal.ROUND_HALF_UP, 2));
				} else {
					acoaslist.add(0);
					salepalllist.add(0);
				}
		...
		if (sumAllall.compareTo(new BigDecimal("0")) != 0) {
			resultMap.put("sumAllsalespall", sumAllSales.multiply(new BigDecimal("100")).divide(sumAllall, BigDecimal.ROUND_HALF_UP, 2));
			resultMap.put("sumAllacoas", sumAllCost.multiply(new BigDecimal("100")).divide(sumAllall, BigDecimal.ROUND_HALF_UP, 2));
		} else {
