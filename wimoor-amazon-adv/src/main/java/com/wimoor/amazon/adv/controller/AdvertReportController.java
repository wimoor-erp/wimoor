...
public class AdvertReportController {
	...
	@Resource
	com.wimoor.amazon.adv.common.service.IAmzAdvReportHandlerService amzAdvReportHandlerService;
	@Resource
	...
	@RequestMapping("/downExcelDate")
	public String downExcelDateAction(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		try {
			...
			try {
				response.sendRedirect("../advReport/noAdvDate.do");
				return null;
	...
	@ResponseBody
	@RequestMapping("/getAdvgroup")
	public Object getAdvGroupAction(HttpServletRequest request){
		UserInfo user = UserInfoContext.get();
		Map<String,Object> map = new HashMap<String, Object>();
		List<AmazonGroup> userAdvGroupList = null;//userService.getCurrentUserAdvGroupList(request);
		List<AmazonGroup> advGroupList =null;// amazonGoupService.findAdvShopNameByUser(user.getCompanyid());
		if(userAdvGroupList == null || userAdvGroupList.size() == 0) {
			map.put("advGroupList", advGroupList);
			map.put("number", "all");
			return map;
		}else {
			map.put("advGroupList", userAdvGroupList);
			if(userAdvGroupList.size() == advGroupList.size()) {
				map.put("number", "all");
			}else {
				map.put("number", "no-all");
			}
			return map;
		}
	}
	...
	@ResponseBody
	@RequestMapping("/getmonthsum")
	public Map<String, Object> getMonthSumAction(HttpServletRequest request, Model model){
	...
	public Result<Map<String, Object>> getSumProductAction(@RequestBody QueryForSumProductDTO dto){
		...
		Map<String, Object> result = amzAdvSumProductAdsService.getDaysSumProduct(param);
