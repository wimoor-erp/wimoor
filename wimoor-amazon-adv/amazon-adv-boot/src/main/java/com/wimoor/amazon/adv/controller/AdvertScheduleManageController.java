...
public class AdvertScheduleManageController {
	...
	public String addSchedulePlanAction(HttpServletRequest request, Model model) {
		...
		int temp = advSchedulePlanService.insertPlan(map);
		if(temp < 3) {
			return "ERROR";
		}
		return "SUCESS";
	...
	public String updateSchedulePlanAction(HttpServletRequest request, Model model) {
		...
		advSchedulePlanService.updatePlan(map);
		return "SUCCESS";
	}
	...
	public String changeSchedulePlanAction(HttpServletRequest request, Model model) {
		...
		map.put("status", status);
		map.put("type", type);
		advSchedulePlanService.updatePlanStatus(map);
		return "SUCCESS";
	}
	...
	public String changePlanRemarkAction(HttpServletRequest request, Model model) {
		...
		map.put("planId", planId);
		map.put("remark", remark);
		advSchedulePlanService.updatePlanRemark(map);
		return "SUCCESS";
	}
	...
	public PageList<Map<String,Object>> getSchedulePlanAction(HttpServletRequest request, Model model) {
		...
		PageList<Map<String, Object>> pageList = advSchedulePlanService.getSchedulePlan(map, ListController.getPageBounds(request));
		if(pageList == null) {
	...
	@ResponseBody
	@RequestMapping("/getSchedulePlanInfo")
	public Object getSchedulePlanInfoAction(HttpServletRequest request, Model model) {
		UserInfo user = UserInfoContext.get();
		String planid = request.getParameter("planid");
		return advSchedulePlanService.getSchedulePlanInfo(user, planid);
	}
