...
public class AmzFinUserItemDataController {
	...
	public Result<Map<String, Object>> saveFinItemDataAction(String marketplaceid,String sku,String groupid,String itemid,String currency,
		...
		int temp = amzFinUserItemDataService.saveFinItemData(params);
		params.remove("user");
		if(temp == 0) {
	...
	@PostMapping("/getFinDataList")
   	public Result<IPage<Map<String,Object>>> getFinDataListAction(@RequestBody AmzFinDataDTO dto) {
		...
		params.put("shopid", user.getCompanyid());
		params.put("endDate", endDate.trim());
	    IPage<Map<String, Object>> list = amzFinUserItemDataService.getFinDataList(dto.getPage(), user, params);
	...
	public Result<?> getFinDataWeekListAction(HttpServletRequest request) {
		UserInfo user = UserInfoContext.get();
		...
		params.put("shopid", user.getCompanyid());
		String ekey = JSON.toJSONString(params);
		List<Map<String, Object>> list = amzFinUserItemDataService.getFinDataLastWeek(ekey,user, params);
		...
		if (list != null) {
			...
			for (Map<String, Object> itemMap : list) {
				...
				fcuurency = ProfitServiceImpl.formatCurrency(currency);
	...
	@RequestMapping(value = "/downFinItemDataTemp")
	public void downFinItemDataTempAction(HttpServletRequest request, HttpServletResponse response, Model model) {
		try {
			// 创建新的Excel工作薄
			UserInfo user = UserInfoContext.get();
	...
	@RequestMapping("/downOtherFee")
	public void downDataExcelByOtherFeeAction(HttpServletRequest request, HttpServletResponse response, Model model)  {
		// 创建新的Excel工作薄
		SXSSFWorkbook workbook = new SXSSFWorkbook();
		// 将数据写入Excel
		UserInfo user = UserInfoContext.get();
		...
		amzFinUserItemDataService.setExcelBookByOtherFee(workbook, list, ftype);
		try {
			response.setContentType("application/force-download");// 设置强制下载不打开
