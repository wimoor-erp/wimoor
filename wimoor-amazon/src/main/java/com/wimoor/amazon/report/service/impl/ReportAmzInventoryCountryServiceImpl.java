		...
		try {
			while ((line = br.readLine()) != null) {
				...
				if(lineNumber!=0) {
					...
					query.eq("country", record.getCountry());
					query.eq("fcondition", record.getFcondition());
					AmzInventoryCountryReport old = amzInventoryCountryReportMapper.selectOne(query);
					if(old!=null) {
						record.setId(old.getId());
						amzInventoryCountryReportMapper.updateById(record);
					}else {
						amzInventoryCountryReportMapper.insert(record);
					}
				}
				lineNumber++;
			}
