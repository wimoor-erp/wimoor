...
//					item-extensions-data
						...
						map.put("purchaseDate", record.getPurchaseDate());
						map.put("amazonauthid", record.getAmazonauthid());
						map.put("sku", record.getSku());
						map.put("salesChannel", record.getSalesChannel());
						map.put("amazonOrderId", record.getAmazonOrderId());
							...
							for (OrdersReport oldrecord : oldrecordlist) {
								if (oldrecord != null) {
									...
									if (compareOrdersReport(record, oldrecord) == false) {
										//amazonOrderAnalysisAgentService.addOrder(oldrecord, record);
										ordersReportMapper.updateById(record);
									}
								}
							}
