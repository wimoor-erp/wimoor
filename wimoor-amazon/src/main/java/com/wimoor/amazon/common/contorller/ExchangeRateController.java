...
public Result<?> getMyCurrencyRate() {
	UserInfo user = UserInfoContext.get();
	return Result.success(iExchangeRateService.getMyCurrencyRate(user.getCompanyid())) ;
}
...
public void downDataExcelByRateAction(HttpServletRequest request, HttpServletResponse response, Model model)  {
	// 创建新的Excel工作薄
	SXSSFWorkbook workbook = new SXSSFWorkbook();
	// 将数据写入Excel
	UserInfo user = UserInfoContext.get();
	List<Map<String, Object>> list = iExchangeRateService.getMyCurrencyRate(user.getCompanyid());
