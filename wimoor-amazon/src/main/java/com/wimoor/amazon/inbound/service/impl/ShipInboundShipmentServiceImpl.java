...
public ShipInboundShipment saveCartShipment(UserInfo user,ShipCartShipmentDTO dto) {
	...
	if(dto.getOpttype()!=null&&"submit".equals(dto.getOpttype())) {
		...
		ByteArrayOutputStream stream2 = createInboundFBAXML(shipment, auth.getSellerid());
		Map<String,Object> map2 = submitfeedService.selectByFeedTypeAndFileName(auth.getId(), inplan.getMarketplaceid(),"POST_FBA_INBOUND_CARTON_CONTENTS",shipment.getShipmentid());
		  //判断状态，上一条请求正在处理中，请稍后再提交;如果请求已经完成，可再次提交。
		  if(map2==null ||map2.get("feedstatus")==null
				||"Error".equals(map2.get("feedstatus").toString())
				||"_CANCELLED_".equals(map2.get("feedstatus").toString())
				||"_DONE_".equals(map2.get("feedstatus").toString())
				||"DONE".equals(map2.get("feedstatus").toString())
				||"CANCELLED".equals(map2.get("feedstatus").toString())){
			AmzSubmitFeedQueue submitfeed = submitfeedService.HandlerFeedQueue(stream2, shipment.getShipmentid(), auth, "POST_FBA_INBOUND_CARTON_CONTENTS", user);
			if(submitfeed!=null) {
...
public ShipInboundShipmenSummarytVo getUnSyncShipment(String groupid, String marketplaceid, String shipmentid, String warehouseid) {
	...
	@Transactional
	public void saveTransTrance(UserInfo user,String shipmentid,String carrier,String transtyle,List<Map<String, Object>> boxinfo){
		ShipInboundShipment shipment = this.getById(shipmentid);
		 ...
		 try {
			 ...
			 if(StrUtil.isNotBlank(qid)) {
				 ...
				 if(que!=null) {
				    ...
				    List<ShipInboundCase> caselist = this.findShipInboundCaseByShipment(shipmentid);
				    ByteArrayOutputStream stream = createInboundFBAExcel(inplan, shipment, volist, boxlist, caselist, auth.getSellerid());
					Map<String,Object> map = submitfeedService.selectByFeedTypeAndFileName(auth.getId(), inplan.getMarketplaceid(),"POST_FLAT_FILE_FROM_EXCEL_FBA_CREATE_CARTON_INFO",shipment.getShipmentid());
					  //判断状态，上一条请求正在处理中，请稍后再提交;如果请求已经完成，可再次提交。
					  if(map==null ||map.get("feedstatus")==null
							||"Error".equals(map.get("feedstatus").toString())
	...
	public void saveSelfTransData(UserInfo user, ShipInboundTrans ship, String operate, List<Map<String, Object>> boxinfo,
		...
		if ("confirm".equals(operate)) {
			saveMarkshiped(user, ship.getShipmentid(),"updateamz");
		}
	...
	public String saveMarkshiped(UserInfo user, String shipmentid,String operateType)   {
        ...
        try {
            if(result!=null&&Result.isSuccess(result)) {
        		...
        		if(auth!=null && market!=null && plan!=null&&!"updateself".equals(operateType)) {
        			saveTransTrance(user,shipmentid,null,null,null);
        		}
