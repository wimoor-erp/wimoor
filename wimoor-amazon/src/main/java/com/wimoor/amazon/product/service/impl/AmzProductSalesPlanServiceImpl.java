...
public IPage<Map<String, Object>> handlePurchase(PlanDTO dto, List<Map<String, Object>> list) {
    		...
    		for(Map<String, Object> item:list) {
    			 try{
					 ...
					 if(msku!=null) {
						 ...
						 mskuMap.put(msku.toString(), item);
					 }
				 }catch(Exception e) {
					 e.printStackTrace();
				 }
    		 ...
    		 if(mskulist.size()>0) {
				 ...
				 if(Result.isSuccess(feignResult)&&feignResult.getData()!=null) {
					 for(Map<String, Object> item:feignResult.getData()) {
						   Map<String, Object> skuinfo = mskuMap.get(item.get("sku").toString());
						   if(skuinfo!=null) {
    ...
    public IPage<Map<String, Object>> handleShip(PlanDTO dto, List<Map<String, Object>> list) {
		 ...
		 try {
			 ...
			 if(mskulist.size()>0&&dto.getPlantype().equals("ship")) {
			  ...
			  IPage<Map<String, Object>> page = dto.getListPage(result);
			  return page;
		 }catch(Exception e) {
			 e.printStackTrace();
...
public void setShipRecord(Map<String,Object> item,String shopid,String marketplaceid,String sku,int puOnDay) {
	...
	public List<Map<String,Object>> ExpandCountryDataByGroup(PlanDetailDTO dto) {
		...
		for(Map<String, Object> item:result) {
			    ...
			    if(dto.getIseu()!=null&&dto.getIseu()==true) {
