...
public class ProductInPresaleController {
    ...
    Result<?> save(@RequestBody List<ProductInPresale> preList) {
    	...
    	for(ProductInPresale item:preList) {
    		...
    		query.eq(ProductInPresale::getSku, item.getSku());
    		query.eq(ProductInPresale::getMarketplaceid, item.getMarketplaceid());
    		query.eq(ProductInPresale::getDate,GeneralUtil.formatDate(item.getDate()) );
    		query.eq(ProductInPresale::getGroupid, item.getGroupid());
			ProductInPresale old = iProductInPresaleService.getOne(query);
			if(item.getQuantity()<0) {
				throw new BizException("预估销量不能是负数");
			}
			if(old!=null) {
				item.setId(old.getId());
				iProductInPresaleService.updateById(item);
			}else {
				iProductInPresaleService.save(item);
			}
			if(sku==null) {
				sku=item.getSku();
