...
public class USProfitServiceImpl extends ProfitServiceImpl {
	...
	public BigDecimal determineOutboundWeight(String country, ProductTier productTier, InputDimensions dimensions,
		...
		if (outboundWeightFormat != null && outboundWeightFormat.getFormat() != null) {
			...
			if(outboundWeight.compareTo(new BigDecimal("1"))>=0) {
				outboundWeight = outboundWeight.setScale(0, BigDecimal.ROUND_CEILING);// 进位取整数，模拟不足一磅按一磅算；
			} else {
				outboundWeight = outboundWeight.multiply(new BigDecimal("16")).setScale(0, BigDecimal.ROUND_CEILING);// 进位取整数，模拟不足一盎司按一盎司算；
				outboundWeight = outboundWeight.divide(new BigDecimal("16"));
			}
		}
	...
	public BigDecimal calculateSmlFBA(String country, String productTierId, BigDecimal outboundWeight,ProfitConfigCountry profitConfigCountry) {
		...
		if(fbaFormat==null) {
			throw new BizException(country+"对于重量【"+outboundWeight+"】找不到对于的FBA费用");
		}
	...
	public BigDecimal calculateStorageFee(String country, ProfitConfigCountry profitConfigX,
		...
		if (inventoryStorageFee != null) {
			...
			BigDecimal volume = inputDimension.getVolume(InputDimensions.unit_in).getValue()
					.divide(new BigDecimal("1728"), 4, BigDecimal.ROUND_HALF_UP);// 立方英寸转换成立方英尺
