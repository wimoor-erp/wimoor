...
public class AmzOrderMainServiceImpl extends ServiceImpl<AmzOrderMainMapper, AmzOrderMain>
		implements IAmzOrderMainService, IAwsSQSMessageHandlerService {
	...
	public void handlerOrderResponse(AmazonAuthority auth, AmzAuthApiTimelimit apilimit, GetOrdersResponse response) {
		...
		if (orders != null && orders.size() > 0) {
			for (int i = 0; i < orders.size(); i++) {
				...
				String channel = order.getFulfillmentChannel() != null ? order.getFulfillmentChannel().getValue()
						: null;
				String tructionid = order.getFulfillmentInstruction() != null
						? order.getFulfillmentInstruction().getFulfillmentSupplySourceId()
						: null;
				...
				BigDecimal ordertotal = order.getOrderTotal() == null ? null
						: new BigDecimal(order.getOrderTotal().getAmount());
				...
				try {
					...
					if (apilimit.apiNotRateLimit()) {
						if (StrUtil.isBlank(token)) {
							ApiCallback<GetOrdersResponse> orderReponseHandler = new ApiCallbackGetOrders(this, auth,
									apilimit, startDate);
	...
	public void runApi(AmazonAuthority amazonAuthority) {
		...
		try {
			List<Marketplace> marketlist = marketplaceService.selectMarketBySellerid(amazonAuthority.getSellerid(),
					amazonAuthority.getShopId());
	...
	public boolean handlerMessage(JSONObject body) {
		...
		AmzOrderMain oldorder = this.getById(amazonOrderId);
		...
		if (oldorderitem != null) {
			if (oldorderitem.getQuantityordered() == null || !oldorderitem.getQuantityordered().equals(quantity)
				...
				if (isshipped && (oldorderitem.getQuantityshipped() == null
						|| !oldorderitem.getQuantityshipped().equals(quantity))) {
