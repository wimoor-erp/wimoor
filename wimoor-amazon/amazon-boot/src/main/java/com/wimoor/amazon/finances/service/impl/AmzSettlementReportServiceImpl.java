...
public class AmzSettlementReportServiceImpl extends ServiceImpl<AmzSettlementReportMapper, AmzSettlementReport> implements IAmzSettlementReportService {
 	...
 	void evaluatorCompileExpressionProfit(Map<String,Object> item,Expression compiledExp,List<Map<String, String>> fieldlist){
 		...
 		for (Map<String, String> itemfield : fieldlist) {
 			if (item.get("field" + itemfield.get("value")) != null) {
 				if (item.get("field" + itemfield.get("value")) == null) {
 					throw new BizException("公式中存在无法识别的字段,请到其他项中修改您的配置");
 				}
	...
	public void addOtherCostMap(Map<String, Object> maps) {
		if (maps != null && maps.get("costDetail") != null) {
			try {
				JSONObject cost = JSONObject.parseObject(maps.get("costDetail").toString());
				BigDecimal othersfee = new BigDecimal("0");
				BigDecimal shipmentfee = cost.getBigDecimal("shipment");
				BigDecimal tax = cost.getBigDecimal("tax");
				BigDecimal transportfee = cost.getBigDecimal("currencyTransportFee");
				BigDecimal marketing = cost.getBigDecimal("marketing");
				BigDecimal other = cost.getBigDecimal("othersFee");
				BigDecimal peritemfee = cost.getBigDecimal("perItemFee");
				BigDecimal otherfee = cost.getBigDecimal("others");
				BigDecimal gst = cost.getBigDecimal("import_GST");
				BigDecimal sgst = cost.getBigDecimal("selling_GST");
				BigDecimal corp = cost.getBigDecimal("corporateInFee");
				BigDecimal vatf = cost.getBigDecimal("vatFee");
				othersfee = tax.add(transportfee).add(marketing).add(other).add(peritemfee).add(otherfee).add(gst).add(sgst).add(corp).add(vatf);
				maps.put("othersfee", othersfee);
				maps.put("shipmentfee", shipmentfee);
				return;
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		if (maps != null && maps.get("avgprice") != null && maps.get("sku") != null && maps.get("marketplaceid") != null&& maps.get("authid") != null && maps.get("asin") != null) {
			BigDecimal sellerprice = (BigDecimal) maps.get("avgprice");
			String authid=maps.get("authid").toString();
			String marketplaceid=maps.get("marketplaceid").toString();
			String groupid=maps.get("groupid").toString();
			String asin=maps.get("asin").toString();
			String sku= maps.get("sku").toString();
			AmazonAuthority authority = amazonAuthorityService.getById(authid);
			QueryWrapper<ProductInfo> query=new QueryWrapper<ProductInfo>();
			query.eq("sku", sku);
			query.eq("marketplaceid", marketplaceid);
			query.eq("amazonAuthId", authid);
			 List<ProductInfo> result = this.iProductInfoService.list(query);
			...
			DimensionsInfo dim=null;
			try {
				Result<Map<String, Object>> re = erpClientOneFeign.getMaterialBySKUAction(sku, authority.getShopId());
				Map<String, Object> mp = re.getData();
				if(mp!=null) {
					costprice=mp.get("price")!=null?new BigDecimal(mp.get("price").toString()):new BigDecimal(0);
					if(mp.get("height")!=null) {
						dim=new DimensionsInfo();
						dim.setHeight(new BigDecimal(mp.get("height").toString()));
						String unit="cm";
						String unitweight="kg";
						if(mp.get("height_unit")!=null) {
							unit=mp.get("height_unit").toString();
						}
						if(mp.get("weight_unit")!=null) {
							unitweight=mp.get("weight_unit").toString();
						}
						dim.setHeightUnits(unit);
						if(mp.get("length")!=null) {
							dim.setLength(new BigDecimal(mp.get("length").toString()));
							dim.setLengthUnits(unit);
						}
						if(mp.get("width")!=null) {
							dim.setWidth(new BigDecimal(mp.get("width").toString()));
							dim.setWidthUnits(unit);
						}
						if(mp.get("weight")!=null) {
							dim.setWeight(new BigDecimal(mp.get("weight").toString()));
							dim.setWeightUnits(unitweight);
						}
			...
			CostDetail cost = iCalculateProfitService.getProfit(info,sellerprice,authority, dim,costprice);
			BigDecimal othersfee = new BigDecimal("0");
			BigDecimal shipmentfee = new BigDecimal("0");
			if (cost != null) {
				BigDecimal tax = cost.getTax().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal transportfee = cost.getCurrencyTransportFee().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal marketing = cost.getMarketing().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal other = cost.getOthers().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal peritemfee = new BigDecimal(0);
				BigDecimal otherfee = cost.getOthersFee().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal gst = cost.getImport_GST().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal sgst = cost.getSelling_GST().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal corp = cost.getCorporateInFee().setScale(2, BigDecimal.ROUND_HALF_UP);
				BigDecimal vatf = cost.getVatFee().setScale(2, BigDecimal.ROUND_HALF_UP);
				othersfee = tax.add(transportfee).add(marketing).add(other).add(peritemfee).add(otherfee).add(gst).add(sgst).add(corp).add(vatf);
				shipmentfee = cost.getShipment();
			}
			maps.put("othersfee", othersfee);
			maps.put("shipmentfee", shipmentfee);
		} else {
			maps.put("othersfee", new BigDecimal("0.00"));
		}
	}
 	...
 	private BigDecimal findSettlementSummarySku_LongtermStorage(Map<String, Object> map) {
 		...
 		for(Map<String, Object> item:result) {
 			...
 			Date snapshot_date=GeneralUtil.getDate(item.get("snapshot_date"));
