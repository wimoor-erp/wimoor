...
public class ExchangeRateHandlerServiceImpl  implements IExchangeRateHandlerService {
	...
	@Override
	public BigDecimal changeCurrencyByLocal(Date day, String from, String to, BigDecimal money) throws BizException {
		if (from == null || to == null) {
			throw new BizException("换算的币别不能为空");
		}
		if (money == null)
		...
		if ("RMB".equals(to))
			to = "CNY";
		ExchangeRate exchangeRateFrom  = exchangeRateService.selectByName(day,from);
		ExchangeRate exchangeRateTo =  exchangeRateService.selectByName(day,to);
