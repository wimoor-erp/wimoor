...
final OrdersFulfilledShipmentsFeeMapper ordersFulfilledShipmentsFeeMapper;
    ...
    private AmzSettlementSummarySkuMonth currencyExchange(
    	...
    	one.setRptStorageFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getRptLongStorageFee(), 2));
    	...
    	//============================================本地成本数据=====================================
    	one.setProfitLocalShipmentfee(exchangeRateHandlerService.changeCurrencyByLocal(shopid,"CNY", tocurrency,item.getProfitVat(), 2));
	...
	private void setExtendsData(AmzSettlementSummarySkuMonth item, Map<String, BigDecimal> storagefee,
		...
		if(advfeeitem!=null) {
			...
			item.setRptAdvUnits(advfeeitem.get("units")!=null?new Integer(advfeeitem.get("units").toString()):new Integer("0"));
		}else {
			item.setRptAdvSpendFee(new BigDecimal("0"));
			item.setRptAdvSales(new BigDecimal("0"));
			item.setRptAdvUnits(new Integer("0"));
		}
	...
	public IPage<Map<String, Object>> findByCondition(Page<Object> page, Map<String, Object> map) {
		...
		if(page.getCurrent()==1) {
			if(result!=null&&result.getRecords()!=null&&result.getRecords().size()>0) {
			    result.getRecords().get(0).put("summary", this.baseMapper.findSummaryByCondition(map));
			}
		}
		return result;
	}
