...
//		currencyUnitList.add("€");
	...
	public Map<String, String> getProductTier(ProfitConfigCountry profitConfigCountry, InputDimensions inputDimension, String country, String isMedia,
		...
		if (profitServiceX != null) {
			...
			isSmlAndLight=checkEuLight(country,isSmlAndLight,price);
	...
	private boolean checkEuLight(String country, boolean isSmlAndLight, BigDecimal price) {
		// TODO Auto-generated method stub
		if(isSmlAndLight==true) {
			if(country.equals("UK")&&price.compareTo(new BigDecimal("10"))>0) {
				throw new BizException("英国售价不在轻小范围，10英镑");
			}else if(country.equals("DE")&&price.compareTo(new BigDecimal("11"))>0) {
				throw new BizException("德国售价不在轻小范围，大于11欧元");
			}else if(("FR,IT,NL,ES").contains(country)&&price.compareTo(new BigDecimal("12"))>0) {
				throw new BizException("法国、意大利，荷兰，西班牙 售价必须小于12欧元");
			}else if(country.equals("SE")&&price.compareTo(new BigDecimal("140"))>0) {
				throw new BizException("瑞典售价不在轻小范围，140克朗");
			}else if(country.equals("PL")&&price.compareTo(new BigDecimal("55"))>0) {
				throw new BizException("波兰售价不在轻小范围，55波兰兹罗提");
			}else if(country.equals("PL")&&price.compareTo(new BigDecimal("10"))>0) {
				throw new BizException("美国售价不在轻小范围，10美元");
			}
		}
		return isSmlAndLight;
	...
	public CostDetail getCostDetail(ProfitQuery query,CostDetail costDetail, int typeId, BigDecimal referralrate, boolean isSmlAndLight,ProfitConfig profitcfg ) {
		...
		if (price != null && price.doubleValue()>0) {
			...
			if(profitcfg!=null) {
				...
				isSmlAndLight=this.checkEuLight(costDetail.getCountry(), isSmlAndLight, price);
