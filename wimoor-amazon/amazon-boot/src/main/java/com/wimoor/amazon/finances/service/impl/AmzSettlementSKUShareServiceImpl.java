...
public class AmzSettlementSKUShareServiceImpl implements IAmzSettlementSKUShareService {
	  ...
	  List<Map<String, Object>> list = amzSettlementReportSummaryDayMapper.summaryFeeBySettment(settlementid);
	    ...
	    for(Map<String, Object> item:list) {
	    	...
	    	if(transaction_type.equals("sku")) {
	    		...
	    		shopfee.shop_quantity_orders= item.get("quantity_orders")!=null?new Integer(item.get("quantity_orders").toString()):shopfee.shop_quantity_orders;
	    		shopfee.shop_sales= item.get("sales")!=null?new Integer(item.get("sales").toString()):shopfee.shop_sales;
  ...
  private void shareShopFeeToSku(ShopFee shopfee,ShopFee shopfeesub,AmzSettlementSummarySku item) {
	    ...
	    if(shopfee.shop_in_principal!=null&&shopfee.shop_in_principal.floatValue()>0.0001) {
	    	rate=item.getPrincipal().divide(shopfee.shop_in_principal,30,BigDecimal.ROUND_HALF_UP);
	    }else if(shopfee.shop_sales!=null&&shopfee.shop_sales>0) {
	    	rate=new BigDecimal(item.getSales()).divide(new BigDecimal(shopfee.shop_sales),30,BigDecimal.ROUND_HALF_UP);
	    }else {
	    	rate=new BigDecimal(0);
	    }
  	    BigDecimal itemStoragefee = shopfee.shop_storage_fee.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
		BigDecimal itemlongStoragefee = shopfee.shop_long_storage_fee.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
	    BigDecimal itemadvspend = shopfee.shop_advspend_fee.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
	    BigDecimal itemcoupon_redemption_fee =shopfee.shop_coupon_redemption_fee.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
	    BigDecimal itemreserve_amount =shopfee.shop_reserve_amount.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
	    BigDecimal itemreimbursement_fee=shopfee.shop_reimbursement_fee.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
	    BigDecimal itemshopother = shopfee.shop_othershare.multiply(rate).setScale(2, BigDecimal.ROUND_HALF_UP);
		        ...
		        */
				...
				if(mycountryconfig!=null) {
					  ...
					  if(mycountryconfig.getCostrate()!=null) {
						 costrate = principal.multiply(mycountryconfig.getCostrate()).divide(new BigDecimal("100"),2,BigDecimal.ROUND_HALF_UP);
					  }
